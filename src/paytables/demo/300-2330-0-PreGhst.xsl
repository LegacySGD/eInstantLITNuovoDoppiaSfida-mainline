			...
			<lxslt:component prefix="my-ext" functions="formatJson,retrievePrizeTable,getType">
				<lxslt:script lang="javascript">
					...
					function formatJson(jsonContext, translations, prizeTable, prizeValues, prizeNamesDesc)
					{
						...
						var scenarioYourNums     = scenario.split('|')[0].split(',');
						var scenarioWinNums 	 = scenario.split('|')[1].split(',');
						...
						var prizeSymbCount = prizeSymbs.split('').map(function(item) {return 0} );

						for (var indexWinNum = 0; indexWinNum < scenarioWinNums.length; indexWinNum++)
						{
							objWinNum = {sValue: '', bMatchedWN: false};
						...
						{
							objYourNum = {sValue: '', sPrize: '', bMatchedWN: false, bMatched3: false};
							...
							for (indexWinNum = 0; indexWinNum < arrWinNums.length; indexWinNum++)
							{
								...
								{
									objYourNum.bMatchedWN = true;
									arrWinNums[indexWinNum].bMatchedWN = true;
								}
							}

							arrYourNums.push(objYourNum);

							prizeSymbCount[prizeSymbs.indexOf(objYourNum.sPrize)]++;

							if (prizeSymbCount[prizeSymbs.indexOf(objYourNum.sPrize)] == 3)
							{
								for (var indexYN = 0; indexYN <= indexYourNum; indexYN++)
								{
									if (arrYourNums[indexYN].sPrize == objYourNum.sPrize)
									{
										arrYourNums[indexYN].bMatched3 = true;
									}
								}
							}
						}
						...
						for (var indexWinNum = 0; indexWinNum < arrWinNums.length; indexWinNum++)
						{
							...
							boxColourStr = (arrWinNums[indexWinNum].bMatchedWN) ? colourLime : colourWhite;
						...
						for (var indexYourNumRow = 0; indexYourNumRow < yourNumRows; indexYourNumRow++)
						{
							...
							for (var indexYourNumCol = 0; indexYourNumCol < yourNumsPerRow; indexYourNumCol++)
							{
								...
								boxColourStr = (arrYourNums[indexYourNum].bMatchedWN) ? colourLime : ((arrYourNums[indexYourNum].bMatched3) ? colourRed : colourWhite);
